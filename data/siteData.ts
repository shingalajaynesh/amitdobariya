...
export interface GalleryItem {
  id: string;
  title: string;
  category: 'speaking' | 'anchoring' | 'students' | 'events' | 'programs' | 'transformation';
  imageUrl: string;
  caption: string;
}
...
export const GALLERY_ITEMS: GalleryItem[] = MEDIA_ITEMS.filter((m) => m.type === 'photo').map((m) => ({
  id: m.id,
  title: m.title,
  category: m.category,
  imageUrl: m.url,
...
export const VIDEOS: VideoItem[] = MEDIA_ITEMS.filter((m) => m.type === 'video').map((m) => ({
  id: m.id,
  title: m.title,
  category: m.category,
  thumbnailUrl: m.thumbnailUrl,
  videoUrl: m.url,
  duration: m.folderCategory.includes('Anchor') ? 'Stage Host' : 'Motivational',
